...
import (
	...
	jump "github.com/panenming/auto_jump"
)

func screenshot() image.Image {
	_, err := exec.Command("adb", "shell", "screencap", "-p", "/sdcard/autojump.png").Output()
	if err != nil {
		log.Fatal("截图失败，请连接手机并请检查开发者选项中的 USB 调试安全设置是否打开。", err)
	}

	// pull这块逻辑需要修改
	cmd, err := exec.Command("adb", "pull", "/sdcard/autojump.png", ".").Output()
	if err != nil {
		log.Fatal("adb pull 错误。", err, cmd)
	}
	f1, err := os.Open("autojump.png")
	if err != nil {
		log.Fatal("读取文件失败。", err)
	}
	defer f1.Close()
	data, _ := ioutil.ReadAll(f1)
	pic, err := png.Decode(bytes.NewReader(data))
	if err != nil {
...
func main() {
	...
	for {
		...
		_, err = exec.Command("adb", "shell", "input", "swipe", touchX, touchY, touchX, touchY,
